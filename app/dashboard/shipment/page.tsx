...
export default function ShipmentPage() {
  ...
  // WOs ready to ship
  const readyWOs = wos.filter((wo) => wo.status === "READY_TO_SHIP");
        ...
        <AccordionList<ShipmentRecord>
          ...
          renderHeader={(shipment) => (
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
              ...
              <div className="flex items-center gap-2 text-txt-secondary font-mono">
                <span>{shipment.woIds?.length || 0} Lệnh WO</span>
              </div>
            </div>
          )}
