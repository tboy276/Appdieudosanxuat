...
async function findTenantRegion() {
  ...
  for (const r of regions) {
    ...
    for (const port of [6543, 5432]) {
      ...
      try {
        ...
        console.log(`\n🎉 SUCCESS! MATCHED REGION POOLER: ${host}:${port}`);
        ...
        // Execute dynamic cleanup & enforce deny (false) on all 13 tables
  ...
  console.log("Region scan completed.");
}

findTenantRegion().catch(console.error);
